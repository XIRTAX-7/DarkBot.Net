...
/// <summary>
/// Полный перезапуск Darkorbit-client (kill + spawn + connect) без остановки бота.
/// </summary>
public sealed class GameClientRestartService : IGameClientRestartAppService
{
        ...
        string? reason = null)
    {
        ...
        try
        {
            ...
            if (launch is null)
            {
                _logger.LogWarning("Cannot restart Darkorbit-client — no launch session");
                return;
            }

            _logger.LogInformation(
                "Restarting Darkorbit-client{Reason}",
            ...
            if (result.Success)
                _logger.LogInformation("Darkorbit-client restart OK — Pepper pid {Pid}", result.PepperPid);
            else
                _logger.LogWarning("Darkorbit-client restart connect failed: {Error}", result.Error);
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
            _logger.LogDebug("Darkorbit-client restart cancelled");
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Darkorbit-client restart failed");
        }
        finally
        {

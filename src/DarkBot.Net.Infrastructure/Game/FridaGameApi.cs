...
public sealed class FridaGameApi : IGameConnection, IGameInstallerProbe, IGameBridgeStatusSource, IDisposable
{
        ...
        out long connectionManagerAddress)
    {
        ...
        mainApplicationAddress = FridaBridgeStatus.ParsePtr(status.MainApplicationAddress);
        mainAddress = FridaBridgeStatus.ParsePtr(status.MainAddress);
        screenManagerAddress = FridaBridgeStatus.ParsePtr(status.ScreenManager);
        connectionManagerAddress = FridaBridgeStatus.ParsePtr(status.ConnectionManager);
        return mainApplicationAddress != 0 && mainAddress != 0 && screenManagerAddress != 0;
    }

...
class Client {
    ...
    shouldLaunchGameMap() {
        ...
        try {
            return new URL(this.arg.dosid).href.search(/(dosid|sid)/) >= 0;
        } catch (err) {
            return false;
        }
    }
    ...
    createWindow(type, url) {
        ...
        if (this.arg.dosid) {
            ...
            const loadWithDosid = async (baseUrl, sidValue, targetUrl) => {
                ...
                await window.loadURL(targetUrl, { userAgent: this.useragent });
            };
            ...
            if (check) {
                ...
                if (sid !== null && baseUrl !== null) {
                    const targetUrl = (this.arg.url && this.arg.url.includes('internalMapRevolution'))
                        ? this.arg.url
                        : `${baseUrl}/indexInternal.es?action=internalMapRevolution`;
                    void loadWithDosid(
                        baseUrl,
                        sid[2],
                        targetUrl
                    ).catch(err => console.error('dosid load failed:', err));
                }
            } else {
                if (this.arg.url) {
                    let baseUrl = new URL(this.arg.url).origin;
                    let targetUrl = this.arg.url;
                    void loadWithDosid(baseUrl, this.arg.dosid, targetUrl)
                        .catch(err => console.error('dosid load failed:', err));
